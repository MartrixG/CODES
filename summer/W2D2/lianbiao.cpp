...
Edge E[M]; int head[N], ecnt = 1;

inline void addlink(int a, int b, int c, int r)
{
	E[++ecnt] = (Edge){b , head[a] , c , r};  head[a] = ecnt;
	E[++ecnt] = (Edge){a , head[b] , -c , 0}; head[b] = ecnt;
}
...
{
	static queue<int> Q;
	for(int i = S; i <= T; i++) ds[i] = inf , cxy[i] = 0;
	Q.push(S); ds[S] = 0; cxy[S] = 1;
	while(!Q.empty())
	{
		int x = Q.front(); Q.pop(); cxy[x] = 0;
		FOR(x) if(E[e].rest && ds[E[e].to] > ds[x] + E[e].c)
		{
...
{
	int flow = inf;
	FORR() flow = min(flow , E[e].rest);
	FORR() E[e].rest -= flow , E[e ^ 1].rest += flow;
