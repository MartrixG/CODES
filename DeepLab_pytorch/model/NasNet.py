...
class NasNetEncoder(nn.Module):
    def __init__(self, stem_filters=96, penultimate_filters=4032, filters_multiplier=2):
        super(NasNetEncoder, self).__init__()
        self.stem_filters = stem_filters
        self.penultimate_filters = penultimate_filters
        self.filters_multiplier = filters_multiplier
        filters = self.penultimate_filters // 24
        ...
        self.conv_left.add_module('relu', nn.ReLU())
        self.conv_left.add_module('conv', nn.Conv2d(2016, 512, 1, stride=1, bias=False))
        self.conv_left.add_module('bn', nn.BatchNorm2d(512, eps=0.001, momentum=0.1, affine=True))

        self.conv_right = nn.Sequential()
        self.conv_right.add_module('relu', nn.ReLU())
        self.conv_right.add_module('conv', nn.Conv2d(2016, 512, 1, stride=1, bias=False))
        self.conv_right.add_module('bn', nn.BatchNorm2d(512, eps=0.001, momentum=0.1, affine=True))

        self.link_conv = nn.Sequential()
        self.link_conv.add_module('conv', nn.Conv2d(in_channels=512, out_channels=512, kernel_size=3, stride=1,
                                                    padding=2, dilation=2, bias=False))
        self.link_conv.add_module('bn', nn.BatchNorm2d(512, eps=0.001, momentum=0.1, affine=True))
    ...
    def features(self, x):
        ...
        return x_cell_10, x_cell_11

    def forward(self, x):
        left_cell, right_cell = self.features(x)
        left_cell = self.conv_left(left_cell)
        right_cell = self.conv_right(right_cell)
        feature = self.link_conv(left_cell + right_cell)
        return self.link_conv(feature)


class NasNetLargeOS16(nn.Module):
    def __init__(self, weight_path):
        super(NasNetLargeOS16, self).__init__()
        pretrained_nasnet = NASNetALarge(num_classes=1001)
        pretrained_nasnet.load_state_dict(torch.load(weight_path))
        nasnet = NasNetEncoder()
        pretrained_dict = pretrained_nasnet.state_dict()
        model_dict = nasnet.state_dict()
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
        model_dict.update(pretrained_dict)
        nasnet.load_state_dict(model_dict)
        self.nasnet = nasnet
