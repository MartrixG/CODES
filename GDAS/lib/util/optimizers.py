...
class _LRScheduler(object):
    ...
    def __repr__(self):
        return ('{name}(warmup={warmup_epochs}, max-epoch={max_epochs}, current::epoch={current_epoch}, '
                'iter={current_iter:.2f}'.format(name=self.__class__.__name__, **self.__dict__) + ', {:})'.format(
            self.extra_repr()))
    ...
    def get_min_info(self):
        lrs = self.get_lr()
        return '#LR=[{:.6f}~{:.6f}] epoch={:03d}, iter={:4.2f}#'.format(min(lrs), max(lrs), self.current_epoch,
                                                                        self.current_iter)
...
class MultiStepLR(_LRScheduler):
    ...
    def extra_repr(self):
        return 'type={:}, milestones={:}, gammas={:}, base-lrs={:}'.format('multistep', self.milestones, self.gammas,
                                                                           self.base_lrs)
...
class LinearLR(_LRScheduler):
    ...
    def extra_repr(self):
        return 'type={:}, max_LR={:}, min_LR={:}, base-lrs={:}'.format('LinearLR', self.max_LR, self.min_LR,
                                                                       self.base_lrs)
...
def get_optim_scheduler(parameters, config):
    assert hasattr(config, 'optim') and hasattr(config, 'scheduler') and hasattr(config,
                                                                                 'criterion'), 'config must have ' \
                                                                                               'optim / scheduler / ' \
                                                                                               'criterion keys ' \
                                                                                               'instead of {:}'.format(
        config)
    if config.optim == 'SGD':
        optim = torch.optim.SGD(parameters, config.LR, momentum=config.momentum, weight_decay=config.decay,
