...
def DenseNet(input_shape=(3, 32, 32), num_class=num_classes, growth_rate=12,
    ...
    for i in range(num_block - 1):
        X, filters = Dense_block(
            X, num_layers, growth_rate, i + 1, droup_out, 4)
        filters = int(filters * 0.5)
        X = transition_block(X, filters, droup_out)

    X, filters = Dense_block(
        X, num_layers, growth_rate, num_block, droup_out, 4)
...
checkpoint = ModelCheckpoint(
    filepath='./cifar10_DenseNet.h5', monitor='val_acc', verbose=1, save_best_only=True)
