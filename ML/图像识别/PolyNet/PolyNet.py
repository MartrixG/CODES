...
def Poly_net(input_shape, num_class):
    ...
    for i in range(3):
        X = block.way_2(X, filters = [(8,24),(8,16)], beta = 0.3, stage = 'A_'+str(i+1))
    X = BatchNormalization()(X)
    X = MaxPooling2D(pool_size = (3, 3), strides = 2, padding = 'same')(X)
    X = Conv2D(filters = 48, kernel_size = 1, strides = 1, padding = 'same',
                kernel_regularizer = l2(1e-4), kernel_initializer = 'he_normal')(X)
    for i in range(6):
        X = block.poly_2(X, filters = [(16,32),(16,24)], beta = 0.3, stage = 'B_'+str(i+1))
        X = block.way_2(X, filters = [(16,32),(16,24)], beta = 0.3, stage = 'B_'+str(i+1))
    X = Conv2D(filters = 48, kernel_size = 1, strides = 1, padding = 'same',
                kernel_regularizer = l2(1e-4), kernel_initializer = 'he_normal')(X)
    X = BatchNormalization()(X)
    X = MaxPooling2D(pool_size = (3, 3), strides = 2, padding = 'same')(X)
    ...
    for i in range(3):
        X = block.poly_2(X, filters = [(32,48),(24,32)], beta = 0.3, stage = 'C_'+str(i+1))
        X = block.way_2(X, filters = [(32,48),(24,32)], beta = 0.3, stage = 'C_'+str(i+1))
    X = AveragePooling2D(pool_size=(4, 4),strides=2,padding='same')(X)

    X = Flatten()(X)
    X = Dense(num_class, activation = 'softmax', kernel_initializer = 'he_normal')(X)
